...
def set_car_type_capacity_amount_in_group(mobj):
    ...
    if mobj.type_dispatch.lower() != "к" and mobj.type_of_car.lower() != "крытый":
        ...
        if (
            to_numeric(mobj.car_dead_weight) >= int("75")
            and mobj.type_of_car.lower() == "полувагон"
        ):
            if mobj.year_for_tariff == "2021":
                for i in range(0, 2):
                    ...
                    pag.press(
                        "tab"
                    )  # курсор переводится к окну с выбором серии полувагона

                if (
                    mobj.specific_van_for_coal_id == "12 196-01"
                    or mobj.specific_van_for_coal_id == "12 196-02"
                ):
                    for i in range(
                        0, 2
                    ):  # данный вагон 1 в списке, но 2 раза стрелку вниз
                        sleep(sleep_short)
                        pag.press("down")
                    sleep(sleep_short)
                    pag.press("enter")
                if mobj.specific_van_for_coal_id == "12 9761-02":
                    for i in range(
                        0, 6
                    ):  # данный вагон 5 в списке, но 6 раз стрелку вниз
                        sleep(sleep_short)
                        pag.press("down")
                    sleep(sleep_short)
                    pag.press("enter")
                ...
                for i in range(0, 2):
                    ...
                    pag.hotkey(
                        "shiftleft", "tab"
                    )  # Курсор возвращается к окну с грузоподъемностью
            else:
